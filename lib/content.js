...
export async function getAdminContent(supabase) {
  const [settingsResult, slidesResult, projectsResult, archiveResult, socialResult] = await Promise.all([
    supabase.from("site_settings").select("key,value").in("key", ["about_kicker", "about_title", "about_paragraphs"]),
    supabase.from("about_slides").select("*").order("position", { ascending: true }),
    supabase.from("projects").select("*").order("position", { ascending: true }),
    supabase.from("design_archive_items").select("*").order("position", { ascending: true }),
  ...
  return {
    ...
    designArchive: archiveResult.data || [],
